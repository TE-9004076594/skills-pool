...
8. **Sync CodeGraph index and show status**

   After implementing tasks, sync the CodeGraph index so subsequent commands see the updated code:
   ```bash
   codegraph sync -q
   ```
   > This ensures `/opsx:verify`, `/opsx:explore`, and other commands operate on fresh index data.
